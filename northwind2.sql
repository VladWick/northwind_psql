...
--
-- Name: employee_audits; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
--
-- Name: order_audits; Type: TABLE; Schema: public; Owner: -; Tablespace: 
--
...
-- employee
select employee_id, last_name, first_name, city, title from employees;
-- employee-territories
select * from employee_territories limit 10;
-- territories
...
-- customers
select customer_id, company_name, contact_title from customers LIMIT 10;
select COUNT(customer_id) from customers LIMIT 10;

-- orders
select order_id, order_date, ship_name from orders LIMIT 10;
select COUNT(order_id) from orders limit 10; 
-- order details 
select * from order_detailes limit 10;
-- products
select product_id, product_name, unit_price from products limit 10;
select COUNT(product_id) from products;
-- categories
select category_id, category_name, description from categories;
-- shippers (грузотправители)
select * from shippers;
-- suppliers (поставщик)
select supplier_id, company_name, address from suppliers LIMIT 10;
...
--
create function getCountOfStates()
returns int 
language plpgsql
as
$$
declare
   us_states_count integer;
begin
   select count(*) 
   into us_states_count
   from us_states;
   
   return us_states_count;
end;
$$;
-- test
select getCountOfStates();


--
-- Function with 1 parameter
--
create or replace function getTerritory_Id_OfTheEmployee(id int)
returns character
language plpgsql
as
$$
declare
   territory_id int;
begin
   select employee_territories.territory_id from employee_territories 
   into territory_id
   where employee_territories.employee_id = id;
...
select territory_id from employee_territories where employee_id = 1;
select getTerritory_Id_OfTheEmployee(1);

--
-- Function with two tables
--
create or replace function getTerritory_OfTheEmployee(id int)
returns character
language plpgsql
as
$$
...
begin
   select territories.territory_description
   from employee_territories, territories
   into territory_description
   where employee_territories.territory_id = territories.territory_id;
   
   return territory_description;
end;
$$;
-- test
select territory_description from territories where territory_id = '06897';
select getTerritory_OfTheEmployee(1);
...
update employees set last_name = 'Wick' where employee_id = 1;
...
-- View without function
create or REPLACE view employee_orders AS (
    select employees.employee_id, orders.order_date, order_id
    from employees, orders
    where employees.employee_id = orders.employee_id
);

-- test
-- last 10 orders of the first employee
select * from employee_orders where employee_id = 1 order by order_date DESC LIMIT 10;


-- 
-- Function: last 10 orders of the employee by id
--
create or replace function getTenLastOrders_OfTheEmployee(id int)
...
begin
    ...
    select *
    from orders
    where id = orders.employee_id
    order by order_date desc
    limit 10;
end;
$$;
-- test
select order_id from getTenLastOrders_OfTheEmployee(1);


-- 
-- Function: last {N} orders of the employee by id
--
create or replace function getLastOrders_OfTheEmployee(id int, amount int)
RETURNS SETOF orders
language plpgsql
as
$$
...
$$;
-- test
select order_id from getLastOrders_OfTheEmployee(1, 20);


-- MAIN VIEW: view with a last two functions
create or REPLACE view products_in_last_orders AS (
...
select * from products_in_last_orders;

-- all the products of the particular order (11077)
select * from products_in_last_orders where order_id = 11077;
-- cost of that order
select SUM(unit_price) from products_in_last_orders where order_id = 11077;

-- Function: cost of order 
create or replace function order_cost(id int)
returns int
language plpgsql
as
$$
...
begin
    ...
    into order_cost
    from products_in_last_orders 
    where order_id = id;

    return order_cost;
end;
$$;
select order_cost(11077) || ' dollars' AS order_price;
...
$$
BEGIN
    INSERT INTO order_audits(order_id, order_date, deleted_on)
    VALUES(OLD.order_id, OLD.order_date, now());
	RETURN OLD;
END;
$$;
-- Trigger
CREATE TRIGGER order_delete
  ...
  ON orders
  FOR EACH ROW
  EXECUTE PROCEDURE log_order_delete();

DROP TRIGGER order_delete on orders;

-- Testing a trigger:
--select COUNT(order_id) from orders;
--select order_id from orders order by order_date desc;
-- 11074
select * from orders where order_id = 11074;
select * from order_details where order_id = 11074;

INSERT INTO orders VALUES (11074, 'SIMOB', 7, '1998-05-06', '1998-06-03', NULL, 2, 18.4400005, 'Simons bistro', 'Vinbæltet 34', 'Kobenhavn', NULL, '1734', 'Denmark');
INSERT INTO order_details VALUES (11074, 16, 17.4500008, 14, 0.0500000007);

DELETE FROM order_details where order_id = 11074;
DELETE FROM orders where order_id = 11074;

select * from order_audits;

-- Report

select 
employee_id, last_name, first_name, city, title 
from employees;

\copy employees 
to
C:/1-must-have/2-tpu/5-semestr/1_databases/6-9-lb-course/northwind/employees.csv
CSV
HEADER;